...
import {positions, normals, uvs, indices} from "../blender/light saber.js"
...
(async () => {
    const cubemap = app.createCubemap({
        ...
        posX: await loadTexture("stormydays_bk.png"),
        negY: await loadTexture("stormydays_bk.png"),
        posY: await loadTexture("stormydays_bk.png"),
        negZ: await loadTexture("stormydays_bk.png"),
        posZ: await loadTexture("stormydays_bk.png")
    });
    ...
    let drawCall = app.createDrawCall(program, vertexArray)
        ...
        .texture("tex", app.createTexture2D(await loadTexture("desert.png")))
    ...
    let mirrorDrawCall = app.createDrawCall(mirrorProgram, mirrorArray)
        ...
        .texture("distortionMap", app.createTexture2D(await loadTexture("desert.png")));
    ...
    function draw() {
        ...
        vec3.rotateY(cameraPosition, vec3.fromValues(clamp(Math.abs(Math.sin(time * .2) * 12), 6, 12 ), 10 + 20 * Math.sin(time * .4), 0), vec3.fromValues(0, 0, 0), time * .1);
